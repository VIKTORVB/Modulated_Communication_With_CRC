...
entity M3_T03_Reconstruct is
    Port ( -- inputs
           ...
           -- outputs
           o_Reconstructed_Data_A: out std_logic_vector(5 downto 0)
          ); 
...
begin
Recive: process(i_Clk,i_CE, i_Rst)
    begin
      if(i_Rst = '1') then -- if reset is high set output to 0
        ...
        if (i_CE = '1') then  -- if clock enable is set
          if(cycle > 0) and (i_Demodulation_Done='1')then -- if cycle is not min and demodulator ready bit is high
            r_Reconstructed_Data(cycle downto cycle -1) <= i_Demodulated_2_Bits_A(1 downto 0); -- push the input to the register at position cycle and cycle -1
            cycle <= cycle-2;                                                                -- decrement cycle by 2
          elsif (cycle < 1) then -- if cycle is at min
            cycle <= 5;           -- reset cycle
            o_Reconstructed_Data_A <= r_Reconstructed_Data(5 downto 0); -- push register to output
          end if;
        end if;
      end if;
    end process;
